...
def get_data(name):
	...
	udata_deptname = str(soup.find_all("div",{'class':'deptname'}))
	get_dept =  re.findall(r'(?<=deptname">).*?(?=</s)',udata_deptname)


	# 获取号码
	get_info = re.findall(r'(?<=class="phone).*?(?=</s)',udata_name)

	# if len(get_info) == 1:
	# 	finst = chin(get_info[0]) + chin(get_dept[0])
	# 	print finst
	# else:
	i = 0
	while i < len(get_info):
		finst =finst + chin(get_info[i])+chin(get_dept[i])
		i = i+1
	return finst
